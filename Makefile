...
mcka_apros-update-db: ## Run migrations Apros container
	docker-compose $(DOCKER_COMPOSE_FILES) exec mcka_apros bash -c 'source /edx/app/apros/venvs/mcka_apros/bin/activate && python manage.py migrate'
...
mcka_apros-shell: ## Run a shell on the APROS container
	...
	docker-compose $(DOCKER_COMPOSE_FILES) exec mcka_apros /bin/bash
...
mcka_apros-static: ## Rebuild static assets for the Apros container
	docker-compose $(DOCKER_COMPOSE_FILES) exec mcka_apros bash -c 'source /edx/app/apros/venvs/mcka_apros/bin/activate && python manage.py assets build && python manage.py collectstatic --noinput'
